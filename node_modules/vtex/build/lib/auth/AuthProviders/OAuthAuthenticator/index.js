...
class OAuthAuthenticator extends AuthProviderBase_1.AuthProviderBase {
    async login(account) {
        ...
        try {
            ...
            if (is_wsl_1.default) {
                api_1.logger.warn("We noticed you're using WSL, in which case you may face login issues depending on your WSL version.");
                api_1.logger.warn(`If you do, make sure your Windows is up to date and try again (${Colors_1.ColorifyConstants.URL_INTERACTIVE('https://support.microsoft.com/en-us/help/4027667/windows-10-update')}).`);
                api_1.logger.warn(`In case login errors persist after updating please create an issue on ${Colors_1.ColorifyConstants.URL_INTERACTIVE('https://github.com/vtex/toolbelt/issues')}. We'll promptly help you find a solution.`);
            }
