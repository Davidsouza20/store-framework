...
class LoginServer {
    ...
    start() {
        return async_retry_1.default(async (bail, attemptNumber) => {
            try {
                ...
                api_1.logger.debug(`LoginServer started on http://${LoginServer.HOSTNAME}:${this.port}`);
            }
            catch (err) {
                api_1.logger.debug(`LoginServer failed to start on port:${this.port}. Reason: ${err.message}.`);
                if (err.code !== 'EADDRINUSE') {
                    return bail(err);
                }
                if (attemptNumber < LoginServer.SERVER_START_RETRIES + 1) {
                    api_1.logger.debug(`Retrying to start LoginServer...`);
                }
                throw ErrorReport_1.ErrorReport.createAndMaybeRegisterOnTelemetry({
                    originalError: err,
                    kind: api_1.ErrorKinds.LOGIN_SERVER_START_ERROR,
    ...
    registerLoginHandler() {
        this.app.use(async (ctx) => {
            ...
            ctx.socket.ref();
            api_1.logger.debug(`Received ${ctx.method} login callback`);
            if (!this.loginState) {
